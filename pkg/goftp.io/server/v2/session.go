...
func (sess *Session) Serve() {
	...
	common.Logger.Info(common.EventNewConnection, zap.String("protocol", "ftp"),
		zap.String("info", fmt.Sprintf("SessionId:%s", sess.id)),
		zap.String("local", sess.conn.LocalAddr().String()),
		zap.String("remote", sess.conn.RemoteAddr().String()))
	...
	sess.Close()
	//common.Logger.Info(common.EventCloseConnection, zap.String("protocol", "ftp"),
	//	zap.String("info", fmt.Sprintf("SessionId:%s", sess.id)),
	//	zap.String("local", sess.conn.LocalAddr().String()),
	//	zap.String("remote", sess.conn.RemoteAddr().String()))
}
...
func (sess *Session) receiveLine(line string) {
	...
	if command != "USER" && command != "PASS" {
		common.Logger.Info(common.EventExecuteCommand,
			zap.String("protocol", "ftp"),
			zap.String("account", sess.LoginUser()),
			zap.String("info", command+" "+param),
			zap.String("local", sess.conn.LocalAddr().String()),
			zap.String("remote", sess.conn.RemoteAddr().String()))
	}
